 ...
 */
export const formatTitle = (route, reTranslate = false) => {
    // 取消 meta 响应式
    ...
    if (reTranslate && !isFunction(title))
        title = translateTitle(String(title), name, useI18n);
 ...
 */
export const translateTitle = (title, name, useI18n = true) => {
    const { router } = serviceConfig;
    const finalTitle = title || name || "no-name";
    if (!router.routeUseI18n || !useI18n)
        return finalTitle;
    // 处理 {{ }}，如 title 为 "{{ _route.Home }}"，则说明 _route.Home 需要 i18n 转化
    if (finalTitle.includes("{{") && finalTitle.includes("}}")) {
        return finalTitle.replace(/({{[\s\S]+?}})/, (_, str) => str.replace(/{{([\s\S]*)}}/, (_, subStr) => t(subStr.trim())));
    }
    if (name) {
