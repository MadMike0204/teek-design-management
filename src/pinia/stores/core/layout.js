...
export const useLayoutStore = defineStore("layoutStore", () => {
    const tabNavList = ref(cacheOperator.getCacheTabNavList() || []);
     ...
     * @param path 路径
     */
    const getTab = (path) => tabNavList.value.find(item => item.path === path);
    /**
     * 根据路由路径找到 tab 信息
     * @param path 路径
     */
    const findTabIndex = (path) => tabNavList.value.findIndex(item => item.path === path);
    /**
     * 获取第一个可关闭 tab 的下标
     */
     ...
     * 新增 tab
     */
    const addTab = async (tab, updateIfExist = false) => {
        const tabNavListValue = tabNavList.value;
        if (tab.meta.hideInTab)
            return;
        // 如果关闭 updateIfExist，则已存在的 tab 不做更新
        if (!updateIfExist && tabNavListValue.some(v => [v.path, v.path + "/"].includes(tab.path)))
            return;
        // 判断动态路由的可打开最大数量
        ...
        if (tabIndex === -1) {
            if (!tab.close) {
                ...
                if (closeTabIndex === -1)
                    ...
                    tabNavList.value.splice(closeTabIndex, 0, tab);
            }
            else
                tabNavListValue.push(tab);
        }
        else
            updateTab(tab);
    };
    ...
    const updateTab = (tab) => {
        ...
        tab.meta ??= {};
        if (tab.title)
            tab.meta.title = tab.title;
        if (tab.icon)
            tab.meta.icon = tab.icon;
        if (index !== -1)
     ...
     * 批量删除 tab
     */
    const removeBatchTab = async (pathList) => {
        pathList.forEach(path => removeTab(path));
    };
    /**
     * 删除左侧 tab
     */
    const removeLeftTab = async (tab) => {
        const index = findTabIndex(tab.path);
        if (index === -1)
            return;
        tabNavList.value = tabNavList.value.filter((item, i) => {
            if (i >= index || item.meta?.isAffix)
                return true;
     ...
     * 关闭右侧 tab
     */
    const removeRightTab = async (tab) => {
        const index = findTabIndex(tab.path);
        if (index === -1)
            return;
        tabNavList.value = tabNavList.value.filter((item, i) => {
     ...
     * 设置 IFrame 缓存
     */
    const setIFame = (iFameList = []) => {
        iframeList.value = iFameList;
    };
    watch(() => tabNav.value.persistence, newValue => handleRecordTabNav(newValue));
    watch(() => tabNavList.value, () => handleRecordTabNav(tabNav.value.persistence), { deep: true });
    const handleRecordTabNav = (needRecord) => {
        if (needRecord)
            cacheOperator.setCacheTabNavList(tabNavList.value);
        else
            cacheOperator.removeCacheTabNavList();
    };
    return {
        tabNavList,
        keepAliveName,
