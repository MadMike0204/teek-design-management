...
export const useTabNav = () => {
    ...
    // 右键激活的 tab
    const rightMenuActiveTab = ref(defaultTab);
    // 菜单功能状态
     ...
     */
    const tabsDragSort = (rootClassName, dragClassName) => {
        // 移动端不支持拖拽
        if (isMobile.value)
            return;
        const tabDom = document.querySelector(rootClassName);
        if (!tabDom)
            return;
     ...
     * 根据路径获取 tab
     */
    const getTabByPath = (path) => layoutStore.getTab(path);
    /**
     * 切换 tab 的关闭与未关闭状态，如果切换为未关闭，则移动到未关闭的 tab 列表末尾
     */
    const toggleFixed = (path) => layoutStore.toggleClose(path);
    /**
     * 根据 route 获取 tab
     */
    const createTabByRoute = (route) => {
        return {
    ...
    const initAffixTabs = () => {
        routeStore.flatRouteList.forEach(item => {
            if (item.meta.isAffix && !item.meta.isFull) {
                const tabParam = createTabByRoute(item);
                layoutStore.addTab(tabParam);
                item.meta.isKeepAlive && layoutStore.addKeepAliveName(item.name);
            }
        });
    };
    /**
    ...
    const addTabByRoute = () => {
        ...
        if (route.name)
            route.meta.isKeepAlive && layoutStore.addKeepAliveName(route.name);
    };
    /**
     * 初始化标签页的右键菜单
     */
    const initContextMenu = (tab) => {
        ...
        if (!tab.close) {
            ...
            if (currentIndex === 0) {
                ...
                multiMenuChange(["refresh", "current", "left", "right", "other", "all"], true);
        }
        // 如果没有选择当前的标签，则不允许刷新
    ...
    const multiMenuChange = (menuList, status) => {
     ...
     * 打开右键菜单，并初始化菜单布局、内容
     */
    const openRightMenu = (e, tab, tabNavRef) => {
        initContextMenu(tab);
     ...
     * 刷新选中的 tab
     */
    const refreshSelectedTab = async (tab) => {
     ...
     * 关闭左侧的 tab
     */
    const closeLeftTab = async (tab) => {
        layoutStore.removeLeftTab(tab);
    };
    /**
     * 关闭右侧的 tab
     */
    const closeRightTab = async (tab) => {
