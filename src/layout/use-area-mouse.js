...
export const useMenuAreaMouse = (offset = 0) => {
    ...
    let menuStopWatcher = null;
    const settingStore = useSettingStore();
    const { x: mouseX } = useMouse();
    const { menu } = storeToRefs(settingStore);
    // 支持子组件手动设置菜单区域状态（菜单折叠触发器组件用到）
    provide(UpdateInMenuAreaStateKey, (state) => (inMenuArea.value = state ?? !inMenuArea.value));
    // 侧边栏（菜单）样式
    ...
    const start = () => {
        menuStopWatcher = watch(mouseX, newVal => {
            ...
            if (showMode === MenuShowModeEnum.AutoHidden && !inMenuArea.value) {
                if (collapsed)
                    targetWidth = collapseWidth / 4;
                else
                    targetWidth = width / 4;
            }
            // 避免鼠标移到菜单外时菜单直接收起，这里添加 30 阈值，整合可以移到折叠菜单触发器位置
            if (newVal > targetWidth + 30 + offset)
                inMenuArea.value = false;
            else
    ...
    watch(() => menu.value.showMode, newVal => {
        // 切换菜单显示模式立即取消之前的监听，然后根据新的菜单显示模式选择是否开启
        stop();
        if (newVal !== MenuShowModeEnum.Static)
            start();
    }, { immediate: true });
    onScopeDispose(() => {
        stop();
...
export const useHeaderAreaMouse = () => {
    ...
    // 内容区元素（支持滚动的元素）
    const pageContentDom = ref(null);
    let headerStopWatcher = null;
    const settingStore = useSettingStore();
    ...
    const topStyle = computed(() => {
        if (inHeaderArea.value) {
            let headerHeight = 0;
            let tabNavHeight = 0;
            if (header.value.enabled)
                headerHeight = header.value.height;
            if (tabNav.value.enabled)
                tabNavHeight = tabNav.value.height;
    ...
    const staticClass = computed(() => {
        if (header.value.showMode === HeaderShowModeEnum.Static)
    ...
    const start = () => {
        ...
        if (header.value.showMode === HeaderShowModeEnum.AutoHidden) {
            headerStopWatcher = watch(mouseY, newValue => {
                ...
                if (newValue > topHeight)
                    inHeaderArea.value = false;
                else
        ...
        if (header.value.showMode === HeaderShowModeEnum.ScrollHidden) {
            onMounted(() => {
                // 初始化监听滚动的元素
                if (!pageContentDom.value)
                    pageContentDom.value = document.querySelector(".page-content");
            });
            headerStopWatcher = watch(scrollY, newValue => {
                ...
                if (newValue < topHeight || arrivedState.top)
                    inHeaderArea.value = true;
                if (directions.top)
                    inHeaderArea.value = true;
                // 向下滚动则认为非顶栏区域状态
    ...
    watch(() => header.value.showMode, newVal => {
        // 切换顶栏显示模式立即取消之前的监听，然后根据新的顶栏显示模式选择是否开启
        stop();
        if (![HeaderShowModeEnum.Static, HeaderShowModeEnum.Fixed].includes(newVal))
            start();
    }, { immediate: true });
    onScopeDispose(() => {
        stop();
