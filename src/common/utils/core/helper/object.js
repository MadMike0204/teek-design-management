 ...
 */
export function get(model, prop) {
 ...
 */
export function set(model, prop, value) {
    if (!model || !isObject(model))
        return false;
 ...
 */
export function removeDuplicateObj(arr, removeKeys, keyIsAndOr = "and") {
    if (!isArray(arr)) {
        console.error("Input is not an array");
        return [];
    }
    if (!arr.length || !removeKeys.length)
        return arr;
    try {
        return arr.reduce((itemArr, next) => {
            const isSame = itemArr.find((item) => {
                if (keyIsAndOr === "and") {
