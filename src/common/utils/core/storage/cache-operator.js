...
class CacheOperator {
    // 标签栏的 tabNav 缓存
    tabNavKey = serviceConfig.cache.tabNavCacheKey;
    // 路由缓存
    dynamicRoutesKey = serviceConfig.cache.cacheDynamicRoutesKey;
    // 缓存 key 前缀
    cacheKeyPrefix = serviceConfig.cache.cacheKeyPrefix;
    // 版本号缓存配置
    versionKey = serviceConfig.cache.versionCacheKey;
    // 缓存版本号的 key
    versionStorageKey = `${this.cacheKeyPrefix}:${this.versionKey}`;
    /**
     * 获取存储的版本号
     */
     ...
     * 设置存储的版本号
     */
    setStoredVersion = (version) => localStorage.setItem(this.versionStorageKey, version);
    /**
    ...
    getItem(key) {
        ...
        if (storedVersion) {
            return (localStorageProxy.getItem(currentVersionKey.replace(`v${localStorageProxy.getVersion()}`, storedVersion), false) ??
     ...
     */
    setCacheTabNavList = (tabNavList) => localStorageProxy.setItem(this.tabNavKey, tabNavList);
    /**
     * 移除标签栏的 tabNav 缓存
     */
     ...
     * 获取路由缓存
     */
    getDynamicRoutes = () => localStorageProxy.getItem(this.dynamicRoutesKey) ?? [];
    /**
     * 设置路由缓存
     *
     * @param dynamicRoutes 路由缓存
     */
    setDynamicRoutes = (dynamicRoutes) => localStorageProxy.setItem(this.dynamicRoutesKey, dynamicRoutes);
    /**
     * 移除路由缓存
     */
