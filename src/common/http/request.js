...
export class Request {
    ...
    constructor(config = {}) {
        ...
        // 指定默认处理器
        const { showLoading = showFullScreenLoading, hideLoading = tryHideFullScreenLoading, showMessage = console.error, } = handlers;
    ...
    initInterceptors() {
        ...
        this.service.interceptors.response.use((response) => {
            ...
            // 用户自定义响应处理
            if (this.interceptors.onResponseProcess)
                return this.interceptors.onResponseProcess(response) ?? data;
            if (this.interceptors.onResponse)
                data = this.interceptors.onResponse(response) ?? data;
            ...
            if (data.code && data.code === ResultEnum.LOGIN) {
                // 处理刷新 token 的逻辑
                if (this.handlers.refreshToken)
                    return this.handleRefreshToken(response);
            ...
            // 根据响应的错误状态码，做不同的处理
            if (error.response)
                this.handlers.showMessage?.(checkStatus(error.response.status), "error");
            return Promise.reject(error);
        });
    }
    getMethod(config) {
     ...
     */
    async handleRefreshToken(response) {
    ...
    processQueue(error) {
        this.failedQueue.forEach(({ resolve, reject, config }) => {
            if (error)
                ...
                this.service(config).then(resolve).catch(reject); // 重新发送请求
        });
        // 清空队列
        this.failedQueue = [];
    }
