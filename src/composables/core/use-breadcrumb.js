...
export const useBreadcrumbs = () => {
    ...
    const getBreadcrumbs = (currentRoute) => {
        ...
        if (!isValidHomeRoute()) {
            ElMessage.error({
                message: "您的首页无法获取，请前往 router/routesConfig.ts，找到 HOME_NAME 变量，将 HOME_NAME 值修改为您首页路由的 name 值",
                duration: 10000,
            });
            return [];
        }
        ...
        // 当前路由的父级路由组成的数组
        let matched = findParentRoutesByPath(currentRoute.meta._fullPath, loadedRouteList);
        currentRoute.meta.title = formatTitle(currentRoute);
        matched.push(currentRoute);
        // 首页加上其他页面
        matched = [homeRoute, ...matched];
        // 过滤掉 hideInBread 的配置
        return matched.filter(item => (item.name || item.meta?.title) && !item.meta?.hideInBread && item.name !== LAYOUT_NAME);
    };
    const stop = watch(() => route.fullPath, () => (breadcrumbList.value = getBreadcrumbs(route)), { immediate: true });
