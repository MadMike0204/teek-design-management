...
export const useBrowserTitle = () => {
    ...
    const getBrowserTitle = () => {
        ...
        // 展示标题的多种模式判断
        if (layout.value.titleMode === TitleModeEnum.ProjectPage)
            return pageTitle ? `${name} - ${pageTitle}` : name;
        if (layout.value.titleMode === TitleModeEnum.UsernamePage) {
            const { username } = userStore.userInfo;
            if (username && pageTitle)
                return `${username} - ${pageTitle}`;
            if (username)
                return `${name} - ${username}`;
            if (!pageTitle)
                return name;
        }
        else if (layout.value.titleMode === TitleModeEnum.Project)
            return name;
        if (layout.value.titleMode === TitleModeEnum.Page)
            return pageTitle + "";
        // 默认标题的模式
