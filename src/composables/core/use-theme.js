...
export const useTheme = () => {
    ...
    const changeGlobalTheme = (theme = themeConfig.value.globalThemeMode) => {
        ...
        if (theme !== globalThemeMode)
            settingStore.$patch({ theme: { globalThemeMode: theme } });
        ...
        if (primaryColor.value)
            changePrimaryColor(primaryColor.value, el);
    ...
    const changePrimaryColor = (color = primaryColor.value, el = document.documentElement) => {
        ...
        if (pc && color !== pc)
            setCssVar(ns.cssVarName(`color-primary`), color, el);
    ...
    const deriveColorByPrimary = (color = primaryColor.value, el = document.documentElement) => {
        // 颜色加深或变浅
        for (let i = 1; i <= 9; i++) {
            setCssVar(ns.cssVarNameEl(`color-primary-light-${i}`), isDark.value ? `${getDarkColor(color, i / 10)}` : `${getLightColor(color, i / 10)}`, el);
        }
        for (let i = 1; i <= 9; i++) {
    ...
    const initTheme = () => {
        changeGlobalTheme();
        if (themeConfig.value.greyMode)
            changeGreyOrWeak(true, "greyMode");
        if (themeConfig.value.weakMode)
            changeGreyOrWeak(true, "weakMode");
    };
    return {
        initTheme,
        changePrimaryColor,
