...
export const useRouteFn = () => {
     ...
     * @param api 接口
     */
    const initDynamicRoutes = async (roles, api) => {
        ...
        if (!routeList) {
            ...
            // 后端控制模式
                ...
                routeList = api ? await getDynamicRoutesFromBackend(api) : [];
            // 前后端控制模式
            else if (isMixedMode)
                routeList = [...authRoutes, ...(api ? await getDynamicRoutesFromBackend(api) : [])];
        }
        if (routeList?.length) {
            // 缓存路由
            if (cacheDynamicRoutes)
                cacheOperator.setDynamicRoutes(routeList);
            // 加载路由
    ...
    const getDynamicRoutesFromStorage = () => {
        if (cacheDynamicRoutes)
            return cacheOperator.getDynamicRoutes();
    };
    /**
     * 从后台接口获取动态路由
     */
    const getDynamicRoutesFromBackend = async (api) => {
     ...
     * @param r 路由对象
     */
    const loadDynamicRoutes = (routers, roles, r = router) => {
        ...
        flatRoutes.forEach(flatRoute => {
            ...
            if (!item.name || !r.hasRoute(item.name)) {
                // 当重置路由后，静态路由也需要二次添加
                if (item.meta.isFull || item.name === LAYOUT_NAME)
                    r.addRoute(item);
                else
                    r.addRoute(LAYOUT_NAME, item);
            }
        });
        // 传到 routeStore 持久化
        routeStore.setRoutes(routes);
        routeStore.setFlatRoutes(flatRoutes);
        routeStore.setHomeRoute(findRouteByName(routes, HOME_NAME));
    };
    /**
     * 过滤出当前系统角色的路由权限
     *
     ...
     * @param roles 权限角色
     */
    const filterOnlyRolesRoutes = (routers, roles) => {
        const rolesRoutes = [];
     ...
     */
    const hasPermission = (router, roles) => {
        if (roles.includes("*"))
            return true;
     ...
     */
    const processRouteMeta = (routers, basePath = "/") => {
        routers.forEach(route => {
            ...
            if (route.meta) {
                ...
                if (useI18n === undefined && routeUseI18n !== undefined)
                    route.meta.useI18n = routeUseI18n;
                if (!isFunction(title)) {
                    route.meta.title = translateTitle(String(title), route.name, route.meta.useI18n);
                }
                if (isKeepAlive === undefined && isKeepAliveGlobal !== undefined)
                    route.meta.isKeepAlive = isKeepAliveGlobal;
                if (isFull === undefined && isFullGlobal !== undefined)
                    route.meta.isFull = isFullGlobal;
                if (useTooltip === undefined && routeUseTooltip !== undefined)
                    route.meta.useTooltip = routeUseTooltip;
            }
            if (route.children && route.children.length) {
                if (isValidURL(fullPath))
                    route.children = processRouteMeta(route.children, "");
                else
                    route.children = processRouteMeta(route.children, fullPath);
            }
        });
        return routers;
    };
    ...
    const processDynamicRoutes = (routers) => {
        ...
        routers.forEach(r => {
            ...
            if (r.children?.length) {
                ...
                if (!r.name && r.children[0].name)
                    r.name = r.children[0].name + "Parent";
            }
            if (r.meta?.iframeOpen && r.meta.iframeSrc && isValidURL(r.meta.iframeSrc))
                r.path = r.meta.iframeSrc;
            else {
                if (r.meta?.iframeKeepAlive && r.meta?.iframeSrc)
                    r.component = IFrameBlank;
                else if (!r.meta?.iframeKeepAlive && r.meta?.iframeSrc)
                    r.component = IFrameView;
                else {
                    ...
                    if (r.component) {
                        if (isType(r.component) === "string")
                            r.component = modules["/src/views" + r.component + ".vue"];
                    }
                    else
                        r.component = modules["/src/views" + r.path + ".vue"];
                }
            }
            if (r.children?.length)
                processDynamicRoutes(r.children);
        });
        return routers;
    };
    /**
     ...
     */
    const filterFlatRoutes = (routeList) => {
        return routeList.reduce((pre, current) => {
            let flatArr = [...pre, current];
            if (current.children)
                flatArr = [...flatArr, ...filterFlatRoutes(current.children)];
            return flatArr;
        }, []);
    };
    /**
     ...
     * @param routeList 路由表
     */
    const ascending = (routeList) => {
        routeList.forEach((r, index) => {
            if (!r.meta)
                r.meta = {};
            // 当 rank 不存在时，根据顺序自动创建，布局路由和首页路由分别在第一位和第二位
            if (r.name === LAYOUT_NAME && !r.meta?.rank)
                r.meta.rank = -1;
            if (r.name === HOME_NAME && !r.meta?.rank)
                r.meta.rank = 0;
            else if (needRank(r))
                r.meta.rank = index + 5;
        });
     ...
     * @param route 路由
     */
    const needRank = (route) => {
        const { name, path, meta = {} } = route;
        if (meta.rank || (meta.rank === 0 && [LAYOUT_NAME, HOME_NAME].includes(name) && path === "/")) {
            return false;
        }
        return true;
    };
     ...
     */
    function findParentRoutesByPath(path, routes, target = "all") {
        // 深度遍历查找
